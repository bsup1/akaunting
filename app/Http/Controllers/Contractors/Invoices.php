...
class Invoices extends Controller
{
    ...
    {
        $invoices = Invoice::with('status')->accrued()->where('customer_id', auth()->user()->contractor->id)->paginate();

        $status = collect(InvoiceStatus::all()->pluck('name', 'code'))
            ->prepend(trans('general.all_type', ['type' => trans_choice('general.statuses', 2)]), '');

        return view('contractors.invoices.index', compact('invoices', 'status'));
    }
    ...
    public function show(Invoice $invoice)
    {
        ...
        return view('contractors.invoices.show', compact('invoice', 'accounts', 'currencies', 'account_currency_code', 'customers', 'categories', 'payment_methods'));
    }
